/* ═══════════════════════════════════════════════════════════════════════════════════════════
   VOLATILIDAD · PRE-EUROPA · DE LA MADRUGADA A LONDRES — v3
   ═══════════════════════════════════════════════════════════════════════════════════════════
   ...
   SÍ CAMBIA — la dinámica. El motor es el mismo que se midió fotograma a fotograma sobre el vídeo
   de IQ Option en Nueva York, y aquí va ADAPTADO A LA MADRUGADA. AVISO HONESTO: de esta franja NO
   hay vídeo, así que su tamaño, su ritmo y su curva salen de conocimiento de mercado. Lo único
   medido es su energía ANTERIOR: 0,3846 eventos/s × 6,8288 pips² = 2,6265 pips²/s — que resultaba
   ser LA MÁS VIOLENTA de las cuatro sesiones, por encima de Nueva York (1,767), en plena madrugada.
   Por eso NO se conserva: se baja a 0,700 —el doble que Asia, el 40 % de Nueva York— que es lo que
   él pidió y lo que deja sitio para que Londres pegue encima durante las cinco horas que se solapan.
   ...
   LO QUE DISTINGUE A ESTA FRANJA. Va de 23:40 a 07:00 y CRUZA LA MEDIANOCHE, así que la curva usa
   orden agresiva, así que menos zarpazos y más rango). Ritmo tres veces más lento. Relojes de
   volatilidad más lentos (90 s / 15 min / 2,5 h). Acoplamiento volatilidad-ritmo más fuerte
   (0,95 frente a 0,6), que es lo que hace que cuando Asia se muere, se muera DEL TODO. Techo de
   espera de 95 s. Y la curva de Tokio en vez de la U de Nueva York.

   CALIBRACIÓN. Se conserva la ENERGÍA (varianza por unidad de tiempo), que es lo que determina
   el recorrido de la sesión. Se conserva la de esta sesión, no la de Nueva York. El mismo
   ...
   FIREBASE. Antes: 10 escrituras por movimiento (~56.000 por noche) más una lectura de
...
const CAL = {
  ...
  VOL_ESCALAS: [
    { semividaS: 60, sd: 0.29 },   // el momento
    { semividaS: 600, sd: 0.32 },  // 10 min — las rachas
    { semividaS: 6600, sd: 0.24 }, // 1,8 h — el humor de la madrugada
  ],
  ...
  RAFAGA_PROB_BASE: 0.1,
  RAFAGA_PROB_EXTRA: 0.35, // se suma en proporción al tamaño, saturando en 4x la mediana
  RAFAGA_MIN_IMPRESIONES: 2,
  RAFAGA_MAX_IMPRESIONES: 6,
  RAFAGA_MS_MIN: 25,
  RAFAGA_MS_MAX: 90,
  ...
  /* ═══ LA PERILLA DE TAMAÑO ═══
     Se lee de Firebase en `config/tamano_pre_europa`. Un número: 1 = como está calibrado, 2 = el doble
...
/* LA MADRUGADA, HORA A HORA (reloj de Bogotá). Esta sesión CRUZA LA MEDIANOCHE, así que las
...
function curvaBase(t) {
  if (t <= CURVA_PREEU[0][0]) return CURVA_PREEU[0][1];
  for (let i = 1; i < CURVA_PREEU.length; i++) {
    if (t <= CURVA_PREEU[i][0]) {
      const [t0, v0] = CURVA_PREEU[i - 1];
      const [t1, v1] = CURVA_PREEU[i];
      return v0 + (v1 - v0) * ((t - t0) / (t1 - t0));
    }
  }
  return CURVA_PREEU[CURVA_PREEU.length - 1][1];
}
...
function factorSesion(hora, minuto) {
  /* EL ESTIRÓN DE MEDIANOCHE. La sesión va de 23:40 a 07:00, así que a las 00:00 el reloj se
     reinicia y la curva daría un salto absurdo hacia atrás. Todo lo que caiga antes del mediodía
     pertenece a la madrugada de DESPUÉS, así que se le suman 24 y la escala queda continua:
     23:40 → 23,67 · 00:30 → 24,5 · 03:00 → 27 · 07:00 → 31. */
  let t = hora + minuto / 60;
  if (t < 12) t += 24;
  return curvaCruda(t) / curvaNorma;
}
...
function sorteaDia(fecha) {
  ...
  sorteaCurva();
  console.log(
    `[PRE-EU] jornada ${fecha} — temperatura ×${Math.exp(volDia).toFixed(2)} · ` +
...
// ══════════════════════════════════════════════════════════════════════════════
/* |Δ| = mediana × σ × e^(s·Z).  LA HORA NO ENTRA AQUÍ, y es deliberado.
   La hora de Tokio decide CADA CUÁNTO se mueve el mercado, jamás CUÁNTO. El tamaño lo eligen
   sólo el azar y la volatilidad —que también es azar—, así que en la hora más muerta de la noche
   el dado conserva exactamente la misma libertad que en la apertura de Tokio. Si en el almuerzo
...
async function imprimir(ref, idx, deltaPrecio) {
  ...
  try {
    ...
    console.error("[PRE-EU] impresión falló:", e.message);
  }
  return null;
}
...
/* LA PERILLA. Vive en Firebase, en `config/tamano_pre_europa`. Se relee cada 5 s en la misma tanda que
...
async function habilitado() {
  ...
  const [sFlag, sTam] = await Promise.all([
    db.ref("config/auto_volatilidad_pre_europa").once("value"),
    db.ref("config/tamano_pre_europa").once("value"),
  ]);
  cfgValor = !!sFlag.val();
  ...
  const nuevo = isFinite(n) && n > 0
    ...
    : 1;
  if (nuevo !== tamano) {
    console.log("[PRE-EU] perilla de tamaño: x" + nuevo.toFixed(2) + " (antes x" + tamano.toFixed(2) + ")");
    tamano = nuevo;
  }
  cfgMs = ahora;
  return cfgValor;
...
function informe() {
  ...
  console.log(
    `📊 [PRE-EU] ${a.length} eventos / ${nImpresiones} impresiones en ${seg.toFixed(0)}s  ` +
...
async function ciclo() {
  ...
  try {
    if (!(await habilitado())) {
      console.log("[PRE-EU] Volatilidad Pre-Europa desactivada (flag)");
      siguienteMs = 5000;
      return;
    }

    const { fecha, hora, minuto } = tsBogota();
    const dentroHorario = hora < 7 || (hora === 23 && minuto >= 40);
    if (!dentroHorario) {
      console.log(`[PRE-EU] Fuera de 23:40-07:00 Bogotá (${hora}:${String(minuto).padStart(2, "0")})`);
      siguienteMs = 10000;
      return;
    }
    ...
    if (!last || typeof last.close !== "number") {
      console.warn("[PRE-EU] Última vela no encontrada o inválida. Reintentando...");
      siguienteMs = 2000;
      return;
    }
    ...
    console.error("[PRE-EU] error en ciclo:", error && error.message ? error.message : error);
    siguienteMs = 5000;
  } finally {
...
console.log(
  `🌅 [PRE-EU] Pre-Europa v3 — lognormal s=${CAL.S_LOGNORMAL}, mediana ${CAL.MEDIANA_PIPS} pips, ` +
