/* ═══════════════════════════════════════════════════════════════════════════════════════════
   VOLATILIDAD · LONDRES · LA SESIÓN MÁS BRAVA DEL DÍA — v3
   ═══════════════════════════════════════════════════════════════════════════════════════════
   ...
   LO QUE DISTINGUE A LONDRES. Es la más AGRESIVA de las cuatro: cola de 1,15 (la más larga, por
...
const CAL = {
  ...
  VOL_ESCALAS: [
    { semividaS: 40, sd: 0.30 },   // el momento
    { semividaS: 420, sd: 0.31 },  // 7 min — las rachas
    { semividaS: 4800, sd: 0.25 }, // 80 min — el humor de la mañana londinense
  ],
  ...
  RAFAGA_PROB_BASE: 0.16,
  RAFAGA_PROB_EXTRA: 0.45, // se suma en proporción al tamaño, saturando en 4x la mediana
  ...
  /* ═══ LA PERILLA DE TAMAÑO ═══
     Se lee de Firebase en `config/tamano_londres`. Un número: 1 = como está calibrado, 2 = el doble
...
/* LA MAÑANA DE LONDRES, HORA A HORA (reloj de Bogotá). Aquí no hay pozo ni almuerzo: esta franja
   empieza arriba del todo y va bajando. Es la sesión más agresiva de las cuatro.
     ...
     02:30  ★ EL PICO DEL DÍA ENTERO. El mayor volumen de las 24 horas.
     03:00  sigue a tope
...
function curvaBase(t) {
  if (t <= CURVA_LON[0][0]) return CURVA_LON[0][1];
  for (let i = 1; i < CURVA_LON.length; i++) {
    if (t <= CURVA_LON[i][0]) {
      const [t0, v0] = CURVA_LON[i - 1];
      const [t1, v1] = CURVA_LON[i];
      return v0 + (v1 - v0) * ((t - t0) / (t1 - t0));
    }
  }
  return CURVA_LON[CURVA_LON.length - 1][1];
}
...
function factorSesion(hora, minuto) {
  /* Londres va de 02:00 a 08:00 y NO cruza la medianoche, así que aquí la hora se usa tal cual.
     (Pre-Europa sí necesita un estirón porque su franja pasa por las 00:00; ésta no.) */
...
function sorteaDia(fecha) {
  ...
  sorteaCurva();
  console.log(
    `[LONDRES] jornada ${fecha} — temperatura ×${Math.exp(volDia).toFixed(2)} · ` +
...
async function imprimir(ref, idx, deltaPrecio) {
  ...
  try {
    ...
    console.error("[LONDRES] impresión falló:", e.message);
  }
  return null;
}
...
/* LA PERILLA. Vive en Firebase, en `config/tamano_londres`. Se relee cada 5 s en la misma tanda que
...
async function habilitado() {
  ...
  const [sFlag, sTam] = await Promise.all([
    db.ref("config/auto_volatilidad_londres").once("value"),
    db.ref("config/tamano_londres").once("value"),
  ]);
  cfgValor = !!sFlag.val();
  ...
  const nuevo = isFinite(n) && n > 0
    ...
    : 1;
  if (nuevo !== tamano) {
    console.log("[LONDRES] perilla de tamaño: x" + nuevo.toFixed(2) + " (antes x" + tamano.toFixed(2) + ")");
    tamano = nuevo;
  }
  cfgMs = ahora;
  return cfgValor;
...
function informe() {
  ...
  console.log(
    `📊 [LONDRES] ${a.length} eventos / ${nImpresiones} impresiones en ${seg.toFixed(0)}s  ` +
...
async function ciclo() {
  ...
  try {
    if (!(await habilitado())) {
      console.log("[LONDRES] Volatilidad Londres desactivada (flag)");
      siguienteMs = 5000;
      return;
    }
    ...
    if (!last || typeof last.close !== "number") {
      console.warn("[LONDRES] Última vela no encontrada o inválida. Reintentando...");
      siguienteMs = 2000;
      return;
    }
    ...
    console.error("[LONDRES] error en ciclo:", error && error.message ? error.message : error);
    siguienteMs = 5000;
  } finally {
...
console.log(
  `🇬🇧 [LONDRES] Londres v3 — lognormal s=${CAL.S_LOGNORMAL}, mediana ${CAL.MEDIANA_PIPS} pips, ` +
