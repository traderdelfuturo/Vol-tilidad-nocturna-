/* ═══════════════════════════════════════════════════════════════════════════════════════════
   VOLATILIDAD · NOCTURNA · SESIÓN DE ASIA — v3
   ═══════════════════════════════════════════════════════════════════════════════════════════
   ...
   de IQ Option en Nueva York, y aquí va ADAPTADO A ASIA. AVISO HONESTO: de Asia NO hay vídeo, así
   que su tamaño, su ritmo y su curva salen de conocimiento de mercado, no de medición. Lo único
   que sí está medido de esta sesión es su ENERGÍA, que se conserva EXACTA: 0,274 eventos/s ×
   1,2602 pips² = 0,3453 pips²/s, la misma de la versión anterior. La noche recorre lo mismo.
   ...
   LO QUE DISTINGUE A ASIA. Cola más corta (s = 1,00 frente a 1,13 de NY: Asia tiene mucha menos
...
const CAL = {
  ...
  RAFAGA_PROB_BASE: 0.07,
  RAFAGA_PROB_EXTRA: 0.28, // se suma en proporción al tamaño, saturando en 4x la mediana
  RAFAGA_MIN_IMPRESIONES: 2,
  RAFAGA_MAX_IMPRESIONES: 5,
  RAFAGA_MS_MIN: 25,
  RAFAGA_MS_MAX: 90,
...
function curvaBase(t) {
  if (t <= CURVA_ASIA[0][0]) return CURVA_ASIA[0][1];
  for (let i = 1; i < CURVA_ASIA.length; i++) {
    if (t <= CURVA_ASIA[i][0]) {
      const [t0, v0] = CURVA_ASIA[i - 1];
      const [t1, v1] = CURVA_ASIA[i];
      return v0 + (v1 - v0) * ((t - t0) / (t1 - t0));
    }
  }
  return CURVA_ASIA[CURVA_ASIA.length - 1][1];
}
...
function sorteaDia(fecha) {
  ...
  sorteaCurva();
  console.log(
    `[ASIA] jornada ${fecha} — temperatura ×${Math.exp(volDia).toFixed(2)} · ` +
...
async function imprimir(ref, idx, deltaPrecio) {
  ...
  try {
    ...
    console.error("[ASIA] impresión falló:", e.message);
  }
  return null;
}
...
function informe() {
  ...
  console.log(
    `📊 [ASIA] ${a.length} eventos / ${nImpresiones} impresiones en ${seg.toFixed(0)}s  ` +
...
async function ciclo() {
  ...
  try {
    if (!(await habilitado())) {
      console.log("[ASIA] Volatilidad Nocturna Asia desactivada (flag)");
      siguienteMs = 5000;
      return;
    }

    const { fecha, hora, minuto } = tsBogota();
    const dentroHorario = (hora >= 18 && hora < 23) || (hora === 23 && minuto <= 40);
    if (!dentroHorario) {
      console.log(`[ASIA] Fuera de 18:00-23:40 Bogotá (${hora}:${String(minuto).padStart(2, "0")})`);
      siguienteMs = 10000;
      return;
    }
    ...
    if (!last || typeof last.close !== "number") {
      console.warn("[ASIA] Última vela no encontrada o inválida. Reintentando...");
      siguienteMs = 2000;
      return;
    }
